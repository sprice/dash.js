...
MediaPlayer.dependencies.BufferController = function() {
    ...
    var STALL_THRESHOLD = .5, requiredQuality = -1, currentQuality = -1, isBufferingCompleted = false, bufferLevel = 0, bufferTarget = 0, criticalBufferLevel = Number.POSITIVE_INFINITY, mediaSource, maxAppendedIndex = -1, lastIndex = -1, type, buffer = null, minBufferTime, hasSufficientBuffer = null, appendedBytesInfo, wallclockTicked = 0, appendingMediaChunk = false, isAppendingInProgress = false, isPruningInProgress = false, inbandEventFound = false, createBuffer = function(mediaInfo) {
        ...
        var self = this, streamId = getStreamId.call(self), chunk;
...
MediaPlayer.dependencies.FragmentController = function() {
    "use strict";
    var fragmentModels = [], findModel = function(context) {
...
MediaPlayer.dependencies.ScheduleController = function() {
    "use strict";
    var fragmentsToLoad = 0, type, ready, fragmentModel, isDynamic, currentRepresentationInfo, initialPlayback = true, isStopped = false, playListMetrics = null, playListTraceMetrics = null, playListTraceMetricsClosed = true, isFragmentLoading = false, timeToloadDelay = 0, validateTimeout, seekTarget = NaN, clearPlayListTraceMetrics = function(endTime, stopreason) {
...
MediaPlayer.dependencies.FragmentModel = function() {
    "use strict";
    var context = null, executedRequests = [], loadingRequests = [], delayLoadingTimeout, loadCurrentFragment = function(request) {
        var self = this;
...
MediaPlayer.rules.BufferLevelRule = function() {
    ...
    var getBufferTarget = function(context) {
        var streamProcessor = context.getStreamProcessor(), streamInfo = context.getStreamInfo(), duration = streamInfo.manifestInfo.duration, isDynamic = streamProcessor.isDynamic(), isLongFormContent = duration >= MediaPlayer.dependencies.BufferController.LONG_FORM_CONTENT_DURATION_THRESHOLD, bufferTarget = NaN;
        if (!isDynamic && this.abrController.isPlayingAtTopQuality(streamInfo)) {
    ...
    return {
        ...
        log: undefined,
        execute: function(context, callback) {
            var mediaInfo = context.getMediaInfo(), mediaType = mediaInfo.type, metrics = this.metricsModel.getReadOnlyMetricsFor(mediaType), bufferLevel = this.metricsExt.getCurrentBufferLevel(metrics) ? this.metricsExt.getCurrentBufferLevel(metrics).level : 0, fragmentCount;
...
MediaPlayer.rules.PlaybackTimeRule = function() {
    ...
    return {
        ...
        log: undefined,
        execute: function(context, callback) {
            var mediaType = context.getMediaInfo().type, mediaInfo = context.getMediaInfo(), streamId = context.getStreamInfo().id, streamProcessor = context.getStreamProcessor(), streamController = streamProcessor.getScheduleController(), representationInfo = streamProcessor.getCurrentRepresentationInfo(), seekTarget = streamController.getSeekTarget(), hasSeekTarget = !isNaN(seekTarget), p = hasSeekTarget ? MediaPlayer.rules.SwitchRequest.prototype.STRONG : MediaPlayer.rules.SwitchRequest.prototype.DEFAULT, keepIdx = !hasSeekTarget, time = hasSeekTarget ? seekTarget : this.adapter.getIndexHandlerTime(streamProcessor), buffer = streamProcessor.bufferController.getBuffer(), appendedChunks, range = null, request;
...
MediaPlayer.rules.ScheduleRulesCollection = function() {
    "use strict";
    var fragmentsToScheduleRules = [], nextFragmentRules = [];
    return {
